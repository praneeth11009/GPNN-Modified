...
from .cad120_config import Paths # Change
# import cad120_config

# import metadata
from .metadata import subactivities, affordances
...
class CAD120(torch.utils.data.Dataset):
    ...
    def __init__(self, feature_data_path, sequence_ids):
        ...
        self.max_node_label_len = np.max([len(subactivities), len(affordances)])
...
def main(args):
    ...
    sequence_ids = pickle.load(open(os.path.join(args.tmp_root, 'cad120_data_list.p'), 'rb'), encoding="latin1")
    # sequence_ids = pickle.load(open(os.path.join(args.tmp_root, 'charades_data_list.p'), 'rb'), encoding="latin1")
    train_num = 80
    val_num = 20
    test_num = 25
