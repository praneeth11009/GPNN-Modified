...
def main(args):
    ...
    # Load data
    training_set, valid_set, testing_set, train_loader, valid_loader, test_loader = more_utils.get_cad_data(args)
    ...
    print('Features sizes edge', edge_feature_size, 'node', node_feature_size)
    ...
    print('Now loading model to cuda')
    if args.cuda:
        model = model.cuda()
    ...
    print('Finish loading model, checkpoint in GPU\n\n\n')
    epoch_errors = list()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print('best_epoch_error: {}, avg_epoch_error: {}\n\n'.format(best_epoch_error,  avg_epoch_error))
...
def train(train_loader, model, criterion, optimizer, epoch, logger, args=None):
    ...
    for i, (edge_features, node_features, adj_mat, node_labels, sequence_ids, node_nums) in enumerate(train_loader):
        ...
        edge_features = more_utils.to_variable(edge_features, args.cuda)
        node_features = more_utils.to_variable(node_features, args.cuda)
        adj_mat = more_utils.to_variable(adj_mat, args.cuda)
        node_labels = more_utils.to_variable(node_labels, args.cuda)
...
def validate(val_loader, model, criterion, logger=None, args=None, test=False):
    ...
    if args.visualize:
        more_utils.plot_all_activity_segmentations(all_sequence_ids, subact_predictions, subact_ground_truth, result_folder)
        more_utils.plot_all_affordance_segmentations(all_sequence_ids, all_node_nums, affordance_predictions, affordance_ground_truth, result_folder)
        ...
        more_utils.plot_confusion_matrix(confusion_matrix, datasets.cad_metadata.subactivities, normalize=True, title='',
        ...
        more_utils.plot_confusion_matrix(confusion_matrix, datasets.cad_metadata.affordances, normalize=True, title='',
...
def parse_arguments():
    ...
    # parser.add_argument('--log-root', default=os.path.join(paths.log_root, 'charades/parsing'), help='log files path')
    # parser.add_argument('--resume', default=os.path.join(paths.tmp_root, 'checkpoints/charades/parsing'), help='path to latest checkpoint')
    ...
    parser.add_argument('--addpose', action='store_true', default=False, help='Set true to use pose features')
    parser.add_argument('--poseonly', action='store_true', default=False, help='Set true to use pose features')
