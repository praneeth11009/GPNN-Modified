...
def get_cad_data(args, prediction=False):
    ...
    if prediction:
        ...
        if args.addpose:
            data_path = os.path.join(args.tmp_root, 'cad120', 'cad120_data_pose.p')
        elif args.poseonly:
            data_path = os.path.join(args.tmp_root, 'cad120', 'cad120_data_pose_1.p')
    ...
    train_loader = torch.utils.data.DataLoader(training_set, collate_fn=utils.collate_fn_cad,
                                               batch_size=args.batch_size,
                                               num_workers=args.prefetch, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, collate_fn=utils.collate_fn_cad,
                                               batch_size=args.batch_size,
                                               num_workers=args.prefetch, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testing_set, collate_fn=utils.collate_fn_cad,
                                              batch_size=args.batch_size,
                                              num_workers=args.prefetch, pin_memory=True)
...
def get_hico_data(args):
    ...
    training_set = HICO(root, train_filenames[:])
    valid_set = HICO(root, val_filenames[:])
    testing_set = HICO(root, test_filenames[:])

    train_loader = torch.utils.data.DataLoader(training_set, collate_fn=utils.collate_fn_hico,
                                               batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.prefetch, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, collate_fn=utils.collate_fn_hico,
                                               batch_size=args.batch_size,
                                               num_workers=args.prefetch, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testing_set, collate_fn=utils.collate_fn_hico,
                                               batch_size=args.batch_size,
                                               num_workers=args.prefetch, pin_memory=True)
    print('Dataset sizes: {} training, {} validation, {} testing.'.format(len(train_loader), len(valid_loader), len(test_loader)))
...
def get_vcoco_data(args):
    ...
    training_set = VCOCO(root, 'train')
    valid_set = VCOCO(root, 'val')
    testing_set = VCOCO(root, 'test')
    train_loader = torch.utils.data.DataLoader(training_set, collate_fn=utils.collate_fn_vcoco,
                                               batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.prefetch, pin_memory=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, collate_fn=utils.collate_fn_vcoco,
                                               batch_size=args.batch_size,
                                               num_workers=args.prefetch, pin_memory=True)
    test_loader = torch.utils.data.DataLoader(testing_set, collate_fn=utils.collate_fn_vcoco,
                                               batch_size=args.batch_size,
                                               num_workers=args.prefetch, pin_memory=True)
    print('Dataset sizes: {} training, {} validation, {} testing.'.format(len(train_loader), len(valid_loader), len(test_loader)))
    return training_set, valid_set, testing_set, train_loader, valid_loader, test_loader
...
def parse_result(result, coco, img_dir, result_folder, img_result_id):
    ...
    for role, color in zip(['obj', 'instr'], [(0, 255, 0), (0, 0, 255)]):
        ...
        if action_role_key in result:
            ...
            if obj_name < 0:
                continue
            obj_name = vcoco_metadata.coco_classes[obj_name]
